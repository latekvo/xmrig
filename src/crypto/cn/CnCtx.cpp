...
void xmrig::CnCtx::create(algo_l3_ctx **ctx, uint8_t *memory, size_t size, size_t count)
{
    for (size_t i = 0; i < count; ++i) {
        auto *c     = static_cast<algo_l3_ctx *>(_mm_malloc(sizeof(algo_l3_ctx), 4096));
        c->memory   = memory + (i * size);

        c->generated_code              = reinterpret_cast<algo_mainloop_fun_ms_abi>(VirtualMemory::allocateExecutableMemory(0x4000, false));
...
void xmrig::CnCtx::release(algo_l3_ctx **ctx, size_t count)
{
    if (ctx[0] == nullptr) {
        return;
    }
